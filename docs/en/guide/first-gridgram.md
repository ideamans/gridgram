...
rendered example next to each snippet. Read top to bottom and you'll
have seen every construct you need for everyday diagrams.
...
and Gridgram renders it to SVG or PNG. Both paths share the same
rendering pipeline; outputs are guaranteed identical.

Before you start, make sure the `gg` binary is on your PATH — see
[Quick start](./) if you haven't installed it yet.
...
`doc { cols: N, rows: M }` statement. See the
[Document](./document/) reference for an example with regions, theming,
and an explicit grid.

---
description: '.gg がなぜそう動くのかを、文法と不変条件で示すリファレンスです。BNF、座標系、解決パイプライン、設定と配列のマージ規則、アイコン解決の優先順位を扱います。'
---
# 仕様
...
選ばれた tier は `NodeLabelResult.tier` でレンダラーまで伝搬され、`DiagramNode` が基準 `leaderGap` をその tier 倍にして描画します — tier 2 / tier 3 で勝った候補が tier 1 の位置に勝手に折り畳まれることはありません。ユーザー側で方向や長さを固定したい場合は、ガイドの [ラベルの方向を固定する](../guide/icon/#ラベルの方向を固定する) を参照してください。`label-direction` / `leader-length` の各 style プロパティで、この候補リストを片方の軸（または 1 通りの組み合わせ）に絞り込めます。

### デモ：8 方向

下図は 5×5 グリッドの中央 3×3 にだけラベルを配置し、デモノードごとに **7 方向の候補をコネクタで埋める** ことで、残った 1 方向にラベルが押し出される様子を可視化したものです。各ラベルの文字（`TR` `MR` `BR` `BC` `BL` `ML` `TL` `TC`）が、そのまま配置器が選んだスロット名と一致します。

<Example name="label-directions" />

左右中央（`ML` / `MR`）は、上下のフォールバックでも収まらないときに最後に試される候補で、コーナーや上下センターが全て塞がる縦長レイアウトで効きます。
